...
if [ "$WITH_NEMO" -eq 1 ]; then
    ...
    if [ -n "${NEMO_CUDA_EXTRA:-}" ]; then
        ...
        uv pip install "nemo_toolkit[asr,$NEMO_CUDA_EXTRA] @ git+https://github.com/NVIDIA/NeMo.git@7ccc79b525f205c2c20595a7dfc927051610962c" "numba>=0.60" "llvmlite>=0.43" "cuda-bindings<13" --python .venv-ml/bin/python
    else
        echo "  Installing NeMo from GitHub with [asr] extras (CPU-only)..."
        uv pip install "nemo_toolkit[asr] @ git+https://github.com/NVIDIA/NeMo.git@7ccc79b525f205c2c20595a7dfc927051610962c" "numba>=0.60" "llvmlite>=0.43" --python .venv-ml/bin/python
    fi
