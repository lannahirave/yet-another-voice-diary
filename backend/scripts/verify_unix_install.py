"""Verify the Linux/macOS full-install environment used by Electron."""
...
def _expected_python(root: Path) -> Path:
    return root / ".venv-ml" / "bin" / "python"
...
def _verify_torch(*, expect_cuda: bool) -> None:
    ...
    _print(
        "Torch: "
        f"{torch.__version__} CUDA={cuda_available} MPS={mps_available}"
    )
